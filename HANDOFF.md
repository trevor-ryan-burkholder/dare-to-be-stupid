...
**State:** `main` at `0.206.0`. The manifests and package-lock root metadata agree. Measured on the
current tree with Node 24.14.1: `npm run lint` and `npm run typecheck` clean; `npm test`
**2693 pass, 0 fail**; `npm run test:integration` **176 pass, 0 fail** (both measured on 0.206.0, with
...
| 0.191.0 | F17 / item 71 | current credit bound to the current test definition |
| 0.192.0 | F19 / item 73 | decision-bearing reads bounded; workspace hashing streamed |
| 0.193.0 | F23, F24 / items 78, 79 | the inert model selector retired; the PRD checkpoint made visible |
| 0.194.0 | item 77 (partial) | the cold-role supply boundary, enforced and manifested at the one door |
| 0.195.0 | F17 / item 71 | the definition rule applied where banking happens, not only where it was computed |
| 0.196.0 | F10 / item 64 | archive before any receipt is written; provisioning throws routed through the door |
| 0.197.0 | F19 / item 73 | the limit made a ceiling on allocation; three remaining first reads bounded |
...
| 0.206.0 | F29 / item 85 | the candidate is evidence to the panel, never authority over it |
