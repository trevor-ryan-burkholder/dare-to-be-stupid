 ...
 * `git reset --hard` it, rewrite `.dare/` and commit over the other. Run 15's result was void.
 *
 ...
 *
 * The lock lives under `.dare/`, so §6's positional rule already protects it: a process marked
 * `DARE_RUNNING` may not write anything there at any depth, which means a builder cannot forge
...
/** The lock's filename inside `.dare/`. */
 ...
 */
export function readRunLock(dareDir) {
  const file = path.join(dareDir, RUN_LOCK_FILE);
  ...
  try {
    ...
    throw new Error(
      `${path.join('.dare', RUN_LOCK_FILE)} exists but could not be read as JSON ` +
  ...
  if (typeof parsed?.pid !== 'number' || !Number.isInteger(parsed.pid)) {
    throw new Error(
      `${path.join('.dare', RUN_LOCK_FILE)} has no usable pid, so it cannot say whether a driver ` +
 ...
 * @returns {void}
 */
export function claimRunLock(dareDir, lock) {
  const file = path.join(dareDir, RUN_LOCK_FILE);
 ...
 * @returns {void}
 */
export function clearRunLock(dareDir) {
  rmSync(path.join(dareDir, RUN_LOCK_FILE), { force: true });
}
